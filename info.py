...
API_ID = int(environ.get('API_ID', '35585958')) # API ID from my.telegram.org
API_HASH = environ.get('API_HASH', '5c3e3e9cca5b0cf55845e0be1410f8b2')  # API Hash from my.telegram.org
...
# ============================
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '5953067512').split()] # Replace with the actual admin ID(s) to add
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1003839613778').split()]  # Channel id for auto indexing (make sure bot is admin)

LOG_CHANNEL = int(environ.get('LOG_CHANNEL', '-1003959231236'))  # Log channel id (make sure bot is admin)
BIN_CHANNEL = int(environ.get('BIN_CHANNEL', '-1003959231236'))  # Bin channel id (make sure bot is admin)
PREMIUM_LOGS = int(environ.get('PREMIUM_LOGS', '-1003959231236'))  # Premium logs channel id
DELETE_CHANNELS = [int(dch) if id_pattern.search(dch) else dch for dch in environ.get('DELETE_CHANNELS', '-1003959231236').split()] #(make sure bot is admin)
support_chat_id = environ.get('SUPPORT_CHAT_ID', '-1004338106754')  # Support group id (make sure bot is admin)
reqst_channel = environ.get('REQST_CHANNEL_ID', '-1004338106754')  # Request channel id (make sure bot is admin)
SUPPORT_CHAT = environ.get('SUPPORT_CHAT', 'https://t.me/+KdBphDGKfKM3MDA1')  # Support group link (make sure bot is admin)

# FORCE_SUB 
auth_req_channels = environ.get("AUTH_REQ_CHANNELS", "-1003938987110")# requst to join Channel for force sub (make sure bot is admin) only for bot ADMINS  
auth_channels     = environ.get("AUTH_CHANNELS", "-1004429996382")# Channels for force sub (make sure bot is admin)

# ============================
# Payment Configuration
# ============================
QR_CODE = environ.get('QR_CODE', 'https://graph.org/file/e419f801841c2ee3db0fc.jpg')    # QR code image for payments
OWNER_UPI_ID = environ.get('OWNER_UPI_ID', 'sitrabanchutia@fam')    # Owner UPI ID for payments
...
COLLECTION_NAME = environ.get('COLLECTION_NAME', 'talkiesflix') # Collection name (default: dreamcinezone_files)
...
LOG_VR_CHANNEL = int(environ.get('LOG_VR_CHANNEL', '-1003959231236')) #Verification Channel Id 
...
# ============================
GRP_LNK = environ.get('GRP_LNK', 'https://t.me/tubi_bots') # Group link for the bot
OWNER_LNK = environ.get('OWNER_LNK', 'https://t.me/tubisupportbot') # Owner link for the bot
UPDATE_CHNL_LNK = environ.get('UPDATE_CHNL_LNK', 'https://t.me/tubi_bots') # Update channel link for the bot
